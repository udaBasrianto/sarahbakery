    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="p-4 max-w-md mx-auto">
        ...
        <Button
          type="button"
          variant="outline"
          className="w-full h-11 rounded-xl font-medium gap-2.5 border-border hover:bg-muted/50 shadow-sm transition-all mb-4 text-sm"
          onClick={async () => {
            if (!googleOAuthEnabled) {
              toast.info("Login Google belum diaktifkan. Silakan gunakan email & password.");
              return;
            }
            setIsLoading(true);
            try {
              ...
              if (!cfg?.data?.enabled) {
                toast.error("Login Google belum diatur admin. Silakan gunakan email/password.");
                setIsLoading(false);
                return;
              }
              const url = `/auth/google/login?redirect_to=${encodeURIComponent("/dashboard")}`;
