      ...
      <div className="w-full max-w-sm">
        ...
        <form onSubmit={handleLogin} className="bg-card rounded-2xl p-6 shadow-card space-y-4">
          ...
          <Button
            type="button"
            variant="outline"
            className="w-full h-11 rounded-xl font-medium gap-2.5 border-border hover:bg-muted/50 shadow-sm transition-all text-sm"
            onClick={async () => {
              if (!googleOAuthEnabled) {
                toast.info("Login Google belum diatur. Silakan gunakan email & password.");
                return;
              }
              setIsLoading(true);
              try {
